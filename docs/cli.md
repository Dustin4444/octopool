...
validated checks need GitHub Actions metadata, the head-filtered run collection and
complete workflow catalogue are then acquired concurrently; requests without Actions
checks skip both metadata collections. Pages within each collection still read in order.
This also applies to PR rollups and fresh watch confirmation. Successful reads use the same logical operation budget;
...
status errors, and run acquisition/identity errors precede catalogue errors. Workflow
associations are built only after both metadata reads settle. Result assembly remains
check-runs before statuses.
