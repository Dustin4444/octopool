...
- **Public-page fallbacks preserve token quota.** Public PR diffs, commit/compare diff and patch media, and explicit-ref content files use token-free GitHub endpoints; supported top-level `gh run list/view` and `gh release view` summaries prefer public pages once anonymous API quota falls below 50%, before Octopool spends a pooled PAT or App token.
