...
const repoBase = "https://github.com/openclaw/octopool";
const repoEditBase = `${repoBase}/edit/main/docs`;
const cname = readCname();
const siteBase = cname ? `https://${cname}` : "";

const productName = "octopool";
const productTagline = "A shared GitHub read relay";
const productDescription =
  "Octopool is a Cloudflare-hosted, org-authenticated GitHub read relay and shared cache for gitcrawl. Trusted OpenClaw members and agents share a pool of GitHub identities for read-heavy maintainer automation, keeping tokens off developer machines.";
...
// Highlighter token range: Unicode private-use area, also untouched by escapeHtml.
...
function llmsTxt() {
  ...
  const docPages = orderedPages.map((page) => `- ${page.title}: ${pageUrl(origin, page.outRel)}`);
  const lines = [
    `# ${productName}`,
    "",
    productDescription,
...
function inline(text, currentRel) {
  ...
  return out.replace(MARK_RE, (_, i) => stash[Number(i)]);
}
...
function homeHero(page) {
  ...
  const startRel = pageMap.get("cli.md")
    ? hrefToOutRel(pageMap.get("cli.md").outRel, page.outRel)
    : "cli.html";
  const features = [
    "Read relay",
    "Shared cache",
  ...
  return `<header class="home-hero">
        <div class="octo-glow" aria-hidden="true"></div>
        <p class="eyebrow">GitHub relay</p>
        <h1>${escapeHtml(productTagline)}</h1>
        <p class="lede">${escapeHtml(description)}</p>
        <div class="home-cta">
          <a class="btn btn-primary" href="${startRel}">Get started</a>
          <a class="btn btn-ghost" href="${repoBase}" rel="noopener">GitHub</a>
          <div class="home-install" aria-label="Install the CLI">
    ...
    <aside class="sidebar">
      <div class="sidebar-head">
        <a class="brand" href="${hrefToOutRel("index.html", page.outRel)}" aria-label="${productName} docs home">
          ${octoMarkSvg()}
          <span><strong>${escapeHtml(productName)}</strong><small>GitHub relay docs</small></span>
        </a>
        ${themeToggleHtml()}
      </div>
      <label class="search"><span>Search</span><input id="doc-search" type="search" placeholder="relay, cache, identities"></label>
...
function restoreStashTokens(value, stash) {
  return value.replace(STASH_RE, (token) => {
...
function highlightShellLine(line) {
  ...
  working = working.replace(
    /\b(octopool|gh|brew|go|git|make|sudo|cd|export|cat|curl|jq|ls|mv|cp|rm|mkdir|wrangler|pnpm|npm|node|dig)\b/g,
    (m) => stashAdd(m, "hl-cmd"),
  );
